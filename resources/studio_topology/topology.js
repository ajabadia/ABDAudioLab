...
function renderStudio() {
    ...
    // 1. Posicionar Target en el centro
    if (topologyData.target) {
        if (!nodePositions.has('target')) {
            nodePositions.set('target', { x: centerX, y: centerY });
        }
    }

    // 2. Posicionar dispositivos (conectados en órbita superior / inactivos en órbita inferior)
    ...
    // Dispositivos conectados: arco superior amplio para evitar aglomeraciones
    connectedDevs.forEach((dev, idx) => {
        if (!nodePositions.has(dev.id)) {
            const count = connectedDevs.length;
            const spanAngle = Math.min(Math.PI * 0.85, Math.max(0.45, (count - 1) * 0.8));
            const startAngle = -Math.PI / 2 - spanAngle / 2;
            const angle = count === 1 ? -Math.PI / 2 : startAngle + (idx / (count - 1)) * spanAngle;
            const radius = 250;
    ...
    // Dispositivos inactivos: arco inferior / laterales con radio extendido
    unassignedDevs.forEach((dev, idx) => {
        if (!nodePositions.has(dev.id)) {
            const count = unassignedDevs.length;
            const spanAngle = Math.min(Math.PI * 0.95, Math.max(0.55, (count - 1) * 0.7));
            const startAngle = Math.PI / 2 - spanAngle / 2;
            const angle = count === 1 ? Math.PI / 2 : startAngle + (idx / (count - 1)) * spanAngle;
            const radius = 340;
            ...
            nodePositions.set(dev.id, {
                x: centerX + radius * Math.cos(angle),
                y: centerY + radius * Math.sin(angle) * 0.75
            });
        }
    });
...
function renderNode(id, dev, isTarget) {
    ...
    badge.textContent = isTarget ? 'MEASUREMENT TARGET' : (dev.assigned ? 'CONNECTED' : 'UNASSIGNED');
    ...
    subtitle.textContent = dev.details || dev.category || (isTarget ? 'Profile: Default Factory Setup' : 'I/O Device');
