                                    ...
                                    const std::vector<MeasuredPoint>& points)
{
    ...
    out << "#include <cstddef>\n";
    out << "#include <cstdint>\n\n";
    out << "#if __has_include(<LutDSP/LutEvaluatorSimd.h>)\n";
    out << "#include <LutDSP/LutEvaluatorSimd.h>\n";
    out << "#elif __has_include(\"dsp/LutEvaluatorSimd.h\")\n";
    out << "#include \"dsp/LutEvaluatorSimd.h\"\n";
    out << "#else\n";
    out << "namespace abd::lutdsp\n";
    out << "{\n";
    ...
    out << "} // namespace abd::lutdsp\n";
    out << "#endif\n\n";
    out << "namespace abd::lutdsp::models\n";
    out << "{\n\n";
    out << "using AbdBatchedPoint = abd::lutdsp::AbdBatchedPoint;\n\n";
    ...
    out << "};\n\n";
    out << "} // namespace abd::lutdsp::models\n\n";
    out << "// Backwards compatibility alias\n";
    out << "namespace abdaudiolab::lut\n";
    out << "{\n";
    out << "    using AbdBatchedPoint = abd::lutdsp::AbdBatchedPoint;\n";
    out << "    using namespace abd::lutdsp::models;\n";
    out << "} // namespace abdaudiolab::lut\n";
    return out.good();
}
                                    ...
                                    const std::vector<MeasuredPoint>& points)
{
    ...
    obsJson["operatorNotes"] = metadata.operatorNotes;
    obsJson["ambientTemperatureC"] = metadata.ambientTemperatureC;
    obsJson["warmupTimeMinutes"] = metadata.warmupTimeMinutes;
    root["laboratoryConditions"] = obsJson;
                                       ...
                                       const std::vector<MeasuredPoint>& points)
{
    ...
    // 1.7.12 Laboratory Observations & Environmental Conditions
    nlohmann::json obsJson;
    obsJson["operatorNotes"] = manifest.operatorNotes;
    obsJson["ambientTemperatureC"] = manifest.ambientTemperatureC;
    obsJson["warmupTimeMinutes"] = manifest.warmupTimeMinutes;
    root["laboratoryConditions"] = obsJson;

    // 5.4 Wiener-Hammerstein LNL System Identification Model
    if (manifest.hasWienerHammersteinModel)
    {
        nlohmann::json whJson;
        whJson["identified"] = true;
        whJson["preFilterH1"] = {
            { "taps", manifest.whH1Taps },
            { "centroidHz", manifest.whPreFilterCentroidHz }
        };
        whJson["staticNonlinearity"] = {
            { "type", "cubic" },
            { "formula", "f(u) = u + a * u^3" },
            { "coeffA", manifest.whNonLinearCoeffA }
        };
        whJson["postFilterH2"] = {
            { "taps", manifest.whH2Taps },
            { "centroidHz", manifest.whPostFilterCentroidHz }
        };
        whJson["goodnessOfFitR2"] = manifest.whGoodnessOfFitR2;
        whJson["residualErrorRms"] = manifest.whResidualErrorRms;
        root["wienerHammersteinModel"] = whJson;
    }
...
bool LutExporter::exportToJavaScriptModule(const std::string& destinationJsPath,
                                          const core::ProfilingMetadata& metadata,
                                          const std::string& tableName,
                                          const std::vector<MeasuredPoint>& points)
{
    std::ofstream out(destinationJsPath);
    if (!out.is_open())
        return false;

    out << "/**\n"
        << " * Generated by ABDAudioLab Look-Up Table (LUT) Exporter\n"
        << " * Target: WebAssembly / WebAudio AudioWorklet (ES6 Module)\n"
        << " * Model: " << metadata.hardwareName << " (" << metadata.targetModule << ")\n"
        << " */\n\n";

    out << "export const " << tableName << "_METADATA = "
        << "{\n"
        << "  hardwareName: \"" << metadata.hardwareName << "\",\n"
        << "  targetModule: \"" << metadata.targetModule << "\",\n"
        << "  pointsCount: " << points.size() << ",\n"
        << "  sampleRate: " << metadata.sampleRate << "\n"
        << "};\n\n";

    out << "// Flattened Float32Array: [param1Norm, param2Norm, mu, sigma, thd]\n";
    out << "export const " << tableName << "_DATA = new Float32Array([\n";
